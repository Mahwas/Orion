...
RECOMMENDED_PRICE_ID = os.getenv("STRIPE_RECOMMENDED_PRICE_ID")  # New price for recommended products
...
class RecommendedCheckoutRequest(BaseModel):
    user_id: str
...
@billing_router.post("/checkout/recommended", response_model=CheckoutResponse)
async def checkout_recommended(payload: RecommendedCheckoutRequest, db: AsyncSession = Depends(get_db)):
    """Create a Stripe Checkout Session for a recommended product purchase."""

    if not RECOMMENDED_PRICE_ID:
        raise HTTPException(
            status_code=500,
            detail="STRIPE_RECOMMENDED_PRICE_ID is not configured. Create a Price in Stripe Dashboard and set the env var.",
        )
    ...
    try:
        session = stripe.checkout.Session.create(
            ...
            customer=customer_id,
            line_items=[
                {"price": RECOMMENDED_PRICE_ID, "quantity": payload.quantity},
            ],
            success_url=SUCCESS_URL + "?session_id={CHECKOUT_SESSION_ID}",
            cancel_url=CANCEL_URL,
            metadata={
                "user_id": payload.user_id,
                "purchase_type": "recommended",
                "product_id": payload.product_id,
            },
        )
    except stripe.StripeError as e:
        logger.error("Stripe error creating recommended checkout: %s", e)
        raise HTTPException(status_code=502, detail="Failed to create checkout session.")
